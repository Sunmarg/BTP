# -*- coding: utf-8 -*-
"""
Created on Sat Nov  7 15:16:51 2020
...
X.set_index('Sizey',inplace=True)
...
train_data = X[:len(X)-75]
test_data = X[len(X)-75:]
...
for i in range(0,len(test_data)):
    result=result+abs(abs(test_data["Deviation"][i]-predictions[0][i])/test_data["Deviation"][i]) if test_data["Deviation"][i]!=0 else 0
...
future_forecast = stepwise_model.predict(n_periods=75)
data=train_data
...
model_fit = model.fit()
# make prediction
yhat1 = model_fit.predict(len(data), len(df)-1)
print(yhat)
yhat1=yhat1.reset_index()
yhat1.drop(["index"],axis=1,inplace=True)
result=0
for i in range(0,len(test_data)):
    result=result+abs(abs(test_data["Deviation"][i]-yhat[0][i])/test_data["Deviation"][i]) if test_data["Deviation"][i]!=0 else 0
result=1-result/len(test_data)
...
estimator = TBATS(seasonal_periods=(p,q))
model = estimator.fit(train_data)
# Forecast 365 days ahead
y_forecast = model.forecast(steps=75)
...
m=df.copy()
n=df.copy()
m.set_index('Sizey',inplace=True)

mtrain_data = m[:len(X)-75]
ntest_data = m[len(X)-75:]
model = VAR(endog=mtrain_data)
model_fit = model.fit()
yhat = model_fit.forecast(model_fit.y, steps=75)
result=0
for i in range(0,len(test_data)):
...
model = VARMAX(mtrain_data, order=(0,2))
model_fit = model.fit(disp=False)
# make prediction
yhat = model_fit.predict(len(train_data), len(df)-1)
print(yhat)
...
#frames = pd.merge(k,AR_forecast,MA_forecast,ARMA_forecast,ARIMA_forecast,SARIMA_forecast,VAR_forecast,VARMA_forecast,SETS_forecast,ETS_forecast,HOLT_forecast)
